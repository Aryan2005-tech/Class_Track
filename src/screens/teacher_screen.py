...
def teacher_tab_manage_subjects():
    ...
    with col2:
        if st.button('Create New Subject', width='stretch', icon=':material/add_circle:'):
    ...
    if subjects:
        for sub in subjects:
            ...
            def make_share_btn(current_sub=sub):
                if st.button(f"Share Code: {current_sub['name']}", key=f"share_{current_sub['subject_code']}", icon=":material/share:"):
...
def teacher_screen_register():
    ...
    with c1:
        header_dashboard()
    with c2:
        if st.button("Go back to Home", type='secondary', key='registerbackbtn', shortcut="control+backspace", icon=':material/arrow_back:'):
            st.session_state['login_type'] = None
            st.rerun()
    ...
    btnc1, btnc2 = st.columns(2)
    with btnc1:
        if st.button('Register Now', icon=":material/person_add:", shortcut="control+enter", width='stretch'):
